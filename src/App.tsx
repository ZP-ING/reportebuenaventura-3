...
import { LogOut, FileText, MapPin, UserCircle, Bell, Building2, BarChart3, Users, Search, Settings, User, Globe, Mail } from 'lucide-react';
...
import { FollowedReports } from './components/FollowedReports';
...
import { syncFollowedReportsFromDB } from './utils/followedReports';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      if (accessToken && refreshToken) {
        supabase.auth.setSession({
          ...
          if (error) {
            ...
            console.log('✅ Supabase session restored for user:', user.email);
            // Sincronizar reportes seguidos después de restaurar la sesión
            syncFollowedReportsFromDB(user.id).catch(err => 
              console.error('Error syncing followed reports:', err)
            );
          }
        });
      }
      ...
      if (user.role !== 'admin') {
        const checkBlockStatus = async () => {
          try {
            ...
            if (response.ok) {
              ...
              if (data.user) {
                // Check if block or suspension status changed
                const statusChanged = 
                  data.user.blocked !== user.blocked || 
                  data.user.suspendedUntil !== user.suspendedUntil;
                
                if (statusChanged) {
                  // Update local user state if block/suspension status changed
                  const updatedUser = { 
                    ...user, 
                  ...
                  // Force reload if user was blocked or suspended
                  if (data.user.blocked || data.user.suspendedUntil) {
                    console.log('Usuario bloqueado/suspendido - recargando aplicación');
                    window.location.reload();
                  }
                }
              }
            } else if (response.status === 403) {
              // User is blocked - force logout and reload
        ...
        // Then check every 10 seconds instead of 30 for faster detection
  ...
  const handleLogin = (user: User) => {
    ...
    // Sincronizar reportes seguidos después del login
    syncFollowedReportsFromDB(user.id).catch(err => 
      console.error('Error syncing followed reports on login:', err)
    );
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-green-50 via-yellow-50 to-lime-50">
              ...
              {!isAdmin && (
                <NotificationBell 
                  ...
                  onNotificationClick={(reportId) => {
                    ...
                    // Optional: scroll to or focus on the specific report
                    // This could be enhanced to open the specific report detail dialog
                  }}
                />
              )}
      ...
      <main className="flex-1 w-full px-6 py-6 md:py-8 flex flex-col min-h-0">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col min-h-0">
          ...
          {!isAdmin && (
            <TabsList className="grid w-full grid-cols-5 mb-8 bg-white/95 backdrop-blur-sm border-2 border-green-200 p-1.5 rounded-2xl shadow-lg">
              <TabsTrigger 
                value="mis-reportes" 
              ...
              >
                ...
                <span className="hidden sm:inline">Reportes Públicos</span>
          ...
          {!isAdmin && (
            <>
              ...
              <TabsContent value="buscar" className="flex-1 min-h-0">
                <PublicReportsTracking currentUser={currentUser} />
              </TabsContent>

              <TabsContent value="seguidos" className="flex-1 min-h-0">
                <FollowedReports currentUser={currentUser} />
              </TabsContent>
