...
export async function classifyReportWithAI(
  ...
  try {
    ...
    if (token) {
      ...
      const result = await aiAPI.classify(title, description, token);
      
      if (result && result.entity && result.confidence) {
        console.log('OpenAI classification successful:', result);
        return {
          entity: result.entity,
          confidence: result.confidence,
          reasoning: result.reasoning || 'Clasificado por IA de OpenAI'
        };
      }
    }
  } catch (error) {
    console.warn('OpenAI classification failed, falling back to keyword-based classification:', error);
  }
