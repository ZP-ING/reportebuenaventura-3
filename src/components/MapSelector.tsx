...
interface MapSelectorProps {
  ...
  manualAddress?: string; // Nueva prop para dirección manual
}
...
export function MapSelector({ location, onLocationChange, onAddressChange, manualAddress }: MapSelectorProps) {
  ...
  // Geocode manual address when it changes
  useEffect(() => {
    if (manualAddress && manualAddress.trim() !== '' && mapLoaded) {
      geocodeAddress(manualAddress);
    }
  }, [manualAddress, mapLoaded]);
  ...
  const geocodeAddress = async (addressQuery: string) => {
    ...
    setIsGeocoding(true);
    try {
      const query = `${addressQuery}, Buenaventura, Valle del Cauca, Colombia`;
  ...
  return (
    <Card className="p-4 bg-gradient-to-br from-green-50 to-lime-50 border-2 border-green-200">
      <div className="space-y-4">
        ...
        <div className="relative w-full h-[500px] md:h-[600px] bg-gradient-to-br from-green-100 to-yellow-100 rounded-[-35px] overflow-hidden border-2 border-green-300 shadow-lg">
          <div 
            ref={mapContainerRef} 
