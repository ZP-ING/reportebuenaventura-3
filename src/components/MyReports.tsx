...
import { reportsAPI, ratingsAPI } from '../utils/api';
...
interface Report {
  ...
  rating?: number;
  userRating?: number; // Calificación del usuario actual
  userRatingComment?: string; // Comentario del usuario actual
  // Compatibilidad
  entity?: string;
  address?: string;
  image?: string;
...
const STATUS_CONFIG = {
  pendiente: { label: 'Pendiente', color: 'bg-yellow-100 text-yellow-800 border-0', icon: Clock },
  'en-proceso': { label: 'En Proceso', color: 'bg-blue-100 text-blue-800 border-0', icon: TrendingUp },
  resuelto: { label: 'Resuelto', color: 'bg-green-100 text-green-800 border-0', icon: CheckCircle2 },
  rechazado: { label: 'Rechazado', color: 'bg-red-100 text-red-800 border-0', icon: AlertCircle },
} as const;
...
export function MyReports({ currentUser }: MyReportsProps) {
  ...
  const handleRatingSubmit = async (rating: number, comment?: string) => {
    ...
    try {
      ...
      // Usar ratingsAPI.add con el comentario
      const commentText = comment || '';
      
      console.log('Enviando calificación:', { rating, comment: commentText, reportId: ratingReport.id }); // Debug log
      
      await ratingsAPI.add(ratingReport.id, rating, commentText, token);
      ...
      const errorMessage = error?.message || 'Error desconocido al guardar la calificación';
                      ...
                      <div className="flex flex-col sm:flex-row gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2 mb-2">
                            ...
                            <Badge className={`${getStatusConfig(report.status).color} px-3 py-1 text-sm`}>
          ...
          {selectedReport && (
            <div className="space-y-4 p-6 flex-1 overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                {/* Ubicación */}
                <div>
                  <h4 className="text-xs text-gray-500 mb-1">Ubicación</h4>
              ...
              {/* User Rating Display or Action */}
              {selectedReport.status === 'resuelto' && (
                <>
                  {selectedReport.userRating ? (
                    <div className="bg-gradient-to-r from-yellow-50 to-orange-50 border-2 border-yellow-200 rounded-lg p-4">
                      <h4 className="text-sm text-gray-700 mb-3">Tu Calificación</h4>
                      <div className="flex items-center gap-2 mb-2">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star
                            key={i}
                            className={`w-6 h-6 ${
                              i < selectedReport.userRating!
                        ...
                        <span className="text-lg ml-2 text-gray-700">
                          {selectedReport.userRating} de 5
                        </span>
                      </div>
                      {selectedReport.userRatingComment && (
                        <p className="text-sm text-gray-600 mt-2">
                          <span className="font-medium">Comentario:</span> {selectedReport.userRatingComment}
                        </p>
                      )}
                    </div>
                  ) : null}
              ...
              <div className="flex flex-wrap gap-3 justify-between items-center">
                <div className="flex gap-2">
                  <ShareButtons
                    ...
                    size="lg"
                  />
                  {selectedReport.status === 'resuelto' && !selectedReport.userRating && (
                    <Button
                      size="lg"
                      onClick={() => {
                ...
                <Button
                  ...
                  variant="outline"
                  onClick={() => setSelectedReport(null)}
                  className="border-green-200 hover:bg-green-50 rounded-[-26px] text-[20px]"
                >
                  Cerrar
                </Button>
              </div>
