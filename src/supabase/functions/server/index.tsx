...
app.post("/make-server-e2de53ff/auth/signin", async (c) => {
  try {
    ...
    if (profile.is_blocked) {
      return c.json({ 
        error: 'USER_BLOCKED',
        message: 'Este usuario ha sido bloqueado de la plataforma'
      }, 403);
    }
    ...
    if (profile.suspended_until) {
      ...
      if (suspendedUntil > now) {
        return c.json({ 
          ...
          message: 'Este usuario ha sido suspendido temporalmente',
          suspensionData: {
            reason: profile.suspension_reason || 'Sin motivo especificado',
    ...
    // Allow login for non-blocked, non-suspended users
    return c.json({
      user: {
        id: profile.id,
        email: profile.email,
...
app.get("/make-server-e2de53ff/auth/me", async (c) => {
  try {
    ...
    // ⭐ Check if user is blocked
    if (profile.is_blocked) {
      console.log('⚠️ Usuario bloqueado intentando acceder:', profile.email);
      return c.json({ 
        error: 'USER_BLOCKED',
    ...
    if (profile.suspended_until) {
      ...
      if (suspendedUntil > now) {
        console.log('⚠️ Usuario suspendido intentando acceder:', profile.email, 'hasta:', suspendedUntil);
        return c.json({ 
          error: 'USER_SUSPENDED',
    ...
    // Include blocked status and suspension info in user info
    return c.json({
      user: {
        id: profile.id,
        email: profile.email,
...
app.get("/make-server-e2de53ff/public/stats", async (c) => {
  try {
    ...
    if (entitiesError) {
      console.error('Error counting entities:', entitiesError);
    }
...
// Delete comment
app.delete("/make-server-e2de53ff/comments/:commentId", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    // Only owner or admin can delete
    if (existingComment.user_id !== auth.userId && !isAdmin(auth)) {
...
// Update comment
app.put("/make-server-e2de53ff/comments/:commentId", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    // Only owner can edit
    if (existingComment.user_id !== auth.userId) {
      return c.json({ error: 'No tienes permiso para editar este comentario' }, 403);
    }
    ...
    if (error) {
      ...
      return c.json({ error: 'Error al actualizar comentario' }, 500);
    }
...
// ==================== REPORT FOLLOWERS ROUTES ====================

// Follow a report
app.post("/make-server-e2de53ff/reports/:reportId/follow", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    if (error) {
      ...
      if (error.code === '23505') {
        return c.json({ message: 'Ya estás siguiendo este reporte', alreadyFollowing: true });
      }
      console.error('Error following report:', error);
      return c.json({ error: 'Error al seguir el reporte' }, 500);
    }
    ...
    return c.json({
      ...
      follow: {
        ...
        reportId: follow.report_id,
        createdAt: follow.created_at
      }
    });
    
  } catch (error) {
    console.error('Follow report error:', error);
    return c.json({ error: 'Error al seguir el reporte' }, 500);
  }
});

// Unfollow a report
app.delete("/make-server-e2de53ff/reports/:reportId/follow", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    if (error) {
      ...
      return c.json({ error: 'Error al dejar de seguir el reporte' }, 500);
    }
    
    return c.json({ message: 'Ya no sigues este reporte' });
    
  } catch (error) {
    console.error('Unfollow report error:', error);
    return c.json({ error: 'Error al dejar de seguir el reporte' }, 500);
  }
});

// Check if user is following a report
app.get("/make-server-e2de53ff/reports/:reportId/follow", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    if (error && error.code !== 'PGRST116') { // PGRST116 is "not found" error
      ...
      return c.json({ error: 'Error al verificar seguimiento' }, 500);
    }
...
// Get all followers of a report (useful for notifications)
app.get("/make-server-e2de53ff/reports/:reportId/followers", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    const { data: followers, error } = await supabase
      ...
      .select(`
        *,
        user:profiles!report_followers_user_id_fkey(id, name, email)
      `)
      .eq('report_id', reportId)
      .order('created_at', { ascending: false });
    ...
    if (error) {
      ...
      return c.json({ error: 'Error al obtener seguidores' }, 500);
    }
    ...
    const transformedFollowers = (followers || []).map((f: any) => ({
      ...
      userName: f.user?.name || 'Usuario',
      userEmail: f.user?.email || '',
      createdAt: f.created_at
    }));
    
    return c.json({ followers: transformedFollowers, count: transformedFollowers.length });
...
// Get all reports followed by a user
app.get("/make-server-e2de53ff/users/:userId/followed-reports", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    // Only allow users to see their own followed reports, or admins to see anyone's
    if (auth.userId !== userId && !isAdmin(auth)) {
      return c.json({ error: 'No autorizado' }, 403);
    }
    ...
    if (error) {
      console.error('Error getting followed reports:', error);
      return c.json({ error: 'Error al obtener reportes seguidos' }, 500);
    }
    
    return c.json({
      followedReports: (follows || []).map((f: any) => ({
        id: f.id,
        userId: f.user_id,
        reportId: f.report_id,
        createdAt: f.created_at
    ...
    console.error('Get followed reports error:', error);
    return c.json({ error: 'Error al obtener reportes seguidos' }, 500);
  }
});
...
app.get("/make-server-e2de53ff/admin/users", async (c) => {
  try {
    ...
    const usersWithCounts = await Promise.all((profiles || []).map(async (profile: any) => {
      ...
      // Check if user is currently suspended (suspended_until is in the future)
...
app.put("/make-server-e2de53ff/admin/users/:id", async (c) => {
  try {
    ...
    const { blocked, suspendedUntil, suspensionReason, role } = await c.req.json();
    ...
    if (blocked !== undefined) {
      console.log(`🔒 Actualizando estado de bloqueo: ${blocked ? 'BLOQUEADO' : 'DESBLOQUEADO'}`);
      ...
      // Update blocked status in profiles
      const { data: updateResult, error: blockedError } = await supabase
        .from('profiles')
        .update({ 
          is_blocked: blocked, 
      ...
      if (blockedError) {
        console.error('❌ Error updating blocked status in DB:', blockedError);
        console.error('❌ Error detalles:', JSON.stringify(blockedError, null, 2));
        return c.json({ error: 'Error al actualizar estado de bloqueo' }, 500);
      }
      
      console.log(`✅ Usuario ${blocked ? 'BLOQUEADO' : 'DESBLOQUEADO'} exitosamente en DB`);
      console.log(`✅ Datos actualizados:`, updateResult);
      
      // Verificar que se guardó correctamente
      const { data: verifyData, error: verifyError } = await supabase
        .from('profiles')
        .select('id, name, email, is_blocked, suspended_until')
        .eq('id', userId)
        .single();
      
      if (!verifyError && verifyData) {
        console.log('✅ VERIFICACIÓN - Datos en la DB ahora:', verifyData);
      }
    }
    
    // Update suspension in profiles
    if (suspendedUntil !== undefined) {
      console.log(`⏰ Actualizando suspensión: ${suspendedUntil ? 'SUSPENDIENDO' : 'LEVANTANDO SUSPENSIÓN'}`);
      ...
      // Add suspension reason and timestamp if suspending
      if (suspendedUntil) {
        updateData.suspension_reason = suspensionReason || 'Sin motivo especificado';
        updateData.suspended_at = new Date().toISOString();
        console.log('📝 Datos de suspensión:', updateData);
      } else {
      ...
      if (suspendError) {
        console.error('❌ Error updating suspension in DB:', suspendError);
        console.error('❌ Error detalles:', JSON.stringify(suspendError, null, 2));
        return c.json({ error: 'Error al actualizar suspensión' }, 500);
      }
      
      console.log(`✅ Suspensión actualizada exitosamente en DB`);
      console.log(`✅ Datos actualizados:`, updateResult);
      
      // Verificar que se guardó correctamente
      const { data: verifyData, error: verifyError } = await supabase
        .from('profiles')
        .select('id, name, email, is_blocked, suspended_until, suspension_reason, suspended_at')
        .eq('id', userId)
        .single();
      ...
      // If suspending, also update auth to prevent login during suspension
      if (suspendedUntil) {
        const suspendDate = new Date(suspendedUntil);
        const now = new Date();
        const hoursUntilExpiry = Math.ceil((suspendDate.getTime() - now.getTime()) / (1000 * 60 * 60));
        
        if (hoursUntilExpiry > 0) {
          const { error: authError } = await supabase.auth.admin.updateUserById(
            userId,
            { ban_duration: `${hoursUntilExpiry}h` }
          );
          
          if (authError) {
            console.error('Error suspending user in auth:', authError);
          }
        }
      } else {
        // Remove suspension from auth
        const { error: authError } = await supabase.auth.admin.updateUserById(
          userId,
          { ban_duration: 'none' }
        );
        ...
        if (authError) {
          console.error('Error removing suspension from auth:', authError);
        }
      }
    }
    ...
    return c.json({ message: 'Usuario actualizado', user: updatedUser });
...
// Classify report using OpenAI (requires auth)
app.post("/make-server-e2de53ff/ai/classify", async (c) => {
  try {
    const auth = await verifyAuth(c.req.header('Authorization'));
    if (!auth) {
      return c.json({ error: 'No autenticado' }, 401);
    ...
    if (!title || !description) {
      return c.json({ error: 'Título y descripción son requeridos' }, 400);
    }
    ...
    if (!openaiApiKey) {
      ...
      return c.json({ error: 'Servicio de clasificación no disponible' }, 503);
    }
    
    // Get all entities to provide as context
    ...
    const entityList = (entities || []).map(e => 
      `- ${e.name} (${e.category}): ${e.description || 'Sin descripción'}`
    ).join('\n');
    ...
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Eres un sistema de clasificación de reportes ciudadanos para la ciudad de Buenaventura, Colombia. Tu tarea es analizar reportes y asignarlos a la entidad gubernamental más apropiada.
...
- El JSON debe tener exactamente esta estructura: {"entity": "Nombre de la Entidad", "confidence": 95, "reasoning": "Razón breve"}
- El nombre de la entidad debe coincidir EXACTAMENTE con uno de la lista anterior
- La confianza debe ser un número entre 0 y 100
- La razón debe ser una explicación breve en español (máximo 100 caracteres)
- NO incluyas texto adicional fuera del JSON
- Considera el contexto colombiano y específicamente de Buenaventura
- Si el reporte puede aplicar a múltiples entidades, elige la más apropiada según la prioridad del problema`
          },
          {
            role: 'user',
            content: `Clasifica este reporte:
...
Responde SOLO con el JSON en el formato especificado.`
    ...
    if (!openaiResponse.ok) {
      ...
      return c.json({ error: 'Error al clasificar con IA' }, 500);
    }
    ...
    if (!aiResponse) {
      ...
      return c.json({ error: 'Error al obtener respuesta de IA' }, 500);
    }
    ...
    try {
      ...
      if (!result.entity || typeof result.confidence !== 'number' || !result.reasoning) {
        console.error('Invalid AI response structure:', result);
        return c.json({ error: 'Respuesta de IA inválida' }, 500);
      }
      
      // Verify entity exists in our database
      const entityExists = entities?.some(e => e.name === result.entity);
      if (!entityExists) {
        console.error('AI suggested non-existent entity:', result.entity);
        // Try to find closest match
        const suggestedEntity = entities?.[0]?.name || 'Alcaldía Municipal';
        result.entity = suggestedEntity;
        result.confidence = Math.max(50, result.confidence - 20);
        result.reasoning = 'Entidad ajustada a opción disponible';
      }
      ...
      return c.json({ error: 'Error al procesar respuesta de IA' }, 500);
    }
    
  } catch (error) {
    console.error('AI classification error:', error);
    return c.json({ error: 'Error al clasificar reporte' }, 500);
  }
});
